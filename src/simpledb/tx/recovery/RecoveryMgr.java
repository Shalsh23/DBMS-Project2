...
public class RecoveryMgr {
    ...
    */
   public int setInt(Buffer buff, int offset, int val) {
      ...
      if (isTempBlock(blk))
         return -1;
    	  ...
    	  if(!buff.isModifiedBy(txnum)){ //saves the buffer if it is dirty
    		  newBlk = buff.saveBlock();
    ...
    * @param newval the value to be written
    */
   public int setString(Buffer buff, int offset, String val) {
	   Block blk = buff.block();
	      Block newBlk;
	      Buffer newBuff;
	      ...
	      if (isTempBlock(blk))
	         return -1;
	    	  ...
	    	  if(!buff.isModifiedBy(txnum)){ //saves the contents of the buffer if it is dirty
	    		  newBlk = buff.saveBlock();
	    		  return new UpdateRecord(txnum, blk, newBlk).writeToLog();
	    	  }
	    	  return -1;
